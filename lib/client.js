...
import { LocalRealtimeAPI } from "./worker-api.js";
...
export class RealtimeClient extends RealtimeEventHandler {
   ...
   */
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug, localModel = false } = {}) {
    super();
    ...
    this.isLocal = localModel;
    this.realtime = this.isLocal ? new LocalRealtimeAPI({ url, debug }) : new RealtimeAPI({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug });
    this.conversation = new RealtimeConversation(this.isLocal);
    this._resetConfig();
  ...
  _addAPIEventHandlers() {
    ...
    this.realtime.on("client.*", (event) => {
      const realtimeEvent = { time: new Date().toISOString(), source: "client", event: event };
      this.dispatch("realtime.event", realtimeEvent);
    });
    this.realtime.on("server.*", (event) => {
      const realtimeEvent = { time: new Date().toISOString(), source: "server", event: event };
      this.dispatch("realtime.event", realtimeEvent);
    });
    ...
    const callTool = async (tool) => {
      try {
        ...
        this.realtime.send("conversation.item.create", { item: { type: "function_call_output", call_id: tool.call_id, output: JSON.stringify(result) } });
      } catch (e) {
        this.realtime.send("conversation.item.create", { item: { type: "function_call_output", call_id: tool.call_id, output: JSON.stringify({ error: e.message }) } });
      }
      this.createResponse();
    };
  ...
  updateSession({
    ...
    const useTools = [].concat(
      (tools || []).map((toolDefinition) => {
        const definition = { type: "function", ...toolDefinition };
      ...
      Object.keys(this.tools).map((key) => {
        return { type: "function", ...this.tools[key].definition };
      }),
    );
      ...
      for (const c of content) {
        if (c.type === "input_audio") {
          if (c.audio instanceof ArrayBuffer || c.audio instanceof Int16Array) {
            c.audio = this.isLocal ? c.audio : RealtimeUtils.arrayBufferToBase64(c.audio);
          }
        }
      }
  ...
  appendInputAudio(arrayBuffer) {
    if (arrayBuffer.byteLength > 0) {
      this.realtime.send("input_audio_buffer.append", { audio: this.isLocal ? RealtimeUtils.arrayBufferToFloat32(arrayBuffer) : RealtimeUtils.arrayBufferToBase64(arrayBuffer) });
