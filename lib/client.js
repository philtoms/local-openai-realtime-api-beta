...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  constructor({
    ...
    // a local model configuration has been supplied.
    this.isLocal = !!modelConfig;
    this.realtime = this.isLocal
      ? new LocalRealtimeAPI({
          url,
          apiKey,
          dangerouslyAllowAPIKeyInBrowser,
          debug,
      ...
      for (const c of content) {
        if (c.type === 'input_audio') {
          if (c.audio instanceof ArrayBuffer || c.audio instanceof Int16Array) {
            c.audio = this.isLocal
              ? c.audio
              : RealtimeUtils.arrayBufferToBase64(c.audio);
          }
        }
      }
      this.realtime.send('conversation.item.create', {
  ...
  appendInputAudio(arrayBuffer) {
    if (arrayBuffer.byteLength > 0) {
      this.realtime.send('input_audio_buffer.append', {
        audio: this.isLocal
          ? RealtimeUtils.arrayBufferToFloat32(arrayBuffer)
          : RealtimeUtils.arrayBufferToBase64(arrayBuffer),
      });
